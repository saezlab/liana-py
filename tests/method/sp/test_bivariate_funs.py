...
def _assert_bivariate(function, desired, mats, dense_weight=False):
...
def test_costine_vectorized(mats):
...
def test_vectorized_jaccard(mats):
...
def test_local_permutation_pvals(pval_mats):
    local_morans = LocalFunction._get_instance('morans')
    local_truth = pval_mats.rng.normal(size=(10, 10))

    pvals = local_morans._permutation_pvals(x_mat=pval_mats.x_mat,
...
def test_local_zscore_pvals(pval_mats):
    local_morans = LocalFunction._get_instance('morans')
    local_truth = pval_mats.rng.normal(size=(10, 10))

    actual = local_morans._zscore_pvals(x_mat=pval_mats.x_mat,
                                        y_mat=pval_mats.y_mat,
                                        weight=pval_mats.weight,
                                        local_truth=local_truth,
                                        mask_negatives=pval_mats.mask_negatives)
...
def test_global_zscore_pvals(pval_mats):
...
def test_global_permutation_pvals(pval_mats):
    global_stat = pval_mats.rng.normal(size=(10))
    pvals = _global_r._permutation_pvals(x_mat=pval_mats.x_mat,
                                         y_mat=pval_mats.y_mat,
