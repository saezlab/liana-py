...
def kang_2018():
    ...
    The data contains ~25k PBMCs cells from 8 pooled patient lupus samples,
    each before and after IFN-beta stimulation.
    ...
    (2018). https://doi.org/10.1038/nbt.4042

    The dataset was preprocessed for and is available via pertpy
    (https://github.com/theislab/pertpy; Heumos et al., In prep.).

    Returns
    -------
    Returns a largely pre-processed AnnData object with the following
    attributes: Raw counts for ~25k cells; ~15k genes; 16 samples; 2 conditions.
    """
    adata = sc.read(
    ...
    adata.obs = adata.obs.rename(
        {"label": "condition", "replicate": "patient"},
        axis=1
    )
    ...
    adata.obs["sample"] = (
        ...
        + "&"
        + adata.obs["patient"].str.slice(8, 13)
    )
