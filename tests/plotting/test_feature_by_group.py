...
def test_feature_by_group(toy_spatial):
    labels = ['Dendritic', 'CD56+ NK']
    _, ax = feature_by_group(
        adata=toy_spatial,
        groupby='bulk_labels',
        labels=labels,
        feature='HES4',
    ...
    for label, layer in zip(labels, ax.collections[1:], strict=True):
        mask = np.asarray(toy_spatial.obs['bulk_labels'] == label)
...
def test_feature_by_group_raises(toy_spatial):
    ...
    with pytest.raises(KeyError, match="'not_a_key' not found in adata.obsm"):
        feature_by_group(adata=toy_spatial, groupby='bulk_labels', labels=['Dendritic'],
                         feature='HES4', spatial_key='not_a_key')
