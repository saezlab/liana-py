...
def test_circle_plot(adata, liana_res):
...
def test_circle_plot_raises(adata, liana_res):
    ...
    with pytest.raises(ValueError, match="`pivot_mode` must be 'counts' or 'mean'"):
        circle_plot(adata, groupby='random', liana_res=liana_res, pivot_mode='neither')

    with pytest.raises(ValueError, match='`score_key` must be provided'):
        circle_plot(adata, groupby='random', liana_res=liana_res,
                    pivot_mode='mean', score_key=None)


def test_get_mask_df(liana_res):
    ...
    either = get_mask_df(pivot_table.copy(), source_cell_type=source,
                              target_cell_type=target, mode='or')
    both = get_mask_df(pivot_table.copy(), source_cell_type=source,
                             target_cell_type=target, mode='and')
    assert (either != 0).sum().sum() > (both != 0).sum().sum()
    assert (both.drop(index=source) == 0).all().all()
    assert (both.drop(columns=target) == 0).all().all()


def test_set_adata_color(adata):
