...
pip install -e ".[extras]" --group dev --group test --group doc
```
...
`liana` is fully typed and checked with [mypy][] in `strict` mode over both `src/` and `tests/`.
The `mypy` hook is part of the `prek` run above, and can also be invoked on its own:

```bash
hatch check types
```

New code is expected to type-check without `# type: ignore` or `typing.cast`.
Where a dependency is untyped, add it to the `[[tool.mypy.overrides]]` module list in `pyproject.toml`
rather than silencing the call site.
...
We also use [bumpversion][] to automatically update the version number in all places and create a git tag. Run one of the following commands in the root of the repository:
