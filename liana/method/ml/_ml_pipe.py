...
def ml_pipe(adata: anndata.AnnData,
    ...
    if output == 'metabolites':
        ...
        lr_res = _get_lr(adata=adata, resource=resource,
                        relevant_cols= rel_cols,
                        base=base, verbose=verbose, met_est=met_est_result)
...
def _join_stats(source, target, dedict, dedict2, resource):
    """
    ...
    """
    source_stats = dedict2[source].copy()
...
def _get_lr(adata, resource, relevant_cols, met_est, base, verbose):
    """
    ...
    # check if genes are ordered correctly
    if not list(met_est.index) == list(dedict2[labels[0]]['names']):
        raise AssertionError("Variable names did not match DE results!")

    # Calculate Mean, logFC and z-scores by group
    for label in labels:
        temp = adata[adata.obs.label.isin([label])]
        dedict2[label]['means'] = scipy.sparse.csr_matrix(met_est.values.T).mean(axis=0).A.flatten()

    # Join Stats
    lr_res = pd.concat(
        [_join_stats(source, target, dedict, dedict2, resource) for source, target in
