...
weight = rng.uniform(size=(20, 20)).astype(np.float32)
...
def test_pc_masked():
    ...
    _assert_bivariate(_masked_pearson, pc_masked_truth, xmat, ymat, weight)


def test_sp_masked():
    sp_masked_truth = np.array([0.23636216, 0.16480756, -0.0148723, 0.22840606, -0.11492944])
    _assert_bivariate(_masked_spearman, sp_masked_truth, xmat, ymat, weight)
...
def test_cosine_masked():
    ...
    _assert_bivariate(_masked_cosine, cosine_masked_truth, xmat, ymat, weight)
...
def test_masked_jaccard():
    ...
    _assert_bivariate(_masked_jaccard, jac_masked_truth, xmat, ymat, weight)
