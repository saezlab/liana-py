...
`anndata` types `.X` as the full union of everything it can store (including
`None` and backed arrays) and `.obs`/`.var` as `DataFrame | Dataset2D`, so the
narrowing helpers from `liana._core._types` are re-exported here for the tests
to use in place of the raw attributes.
"""
...
def not_none[T](value: T | None) -> T:
    ...
    Many of liana's entry points return `None` when `inplace=True` and a value
    otherwise; a test that passes `inplace=False` knows which it got.
    """
...
def as_anndata(value: object) -> AnnData:
    ...
    liana's entry points are typed for every shape they can return (`AnnData`,
    a frame, or `None` when `inplace=True`); a given call site knows which.
    """
...
def plot_data(plot: object) -> DataFrame:
    ...
    `ggplot.data` is `Optional[DataLike]`, so it needs narrowing before a test
    can index it.
    """
...
def invalid(value: object) -> Any:
    ...
    Several public parameters are `Literal`s yet still validate at runtime, for
    callers that are not themselves type-checked. The tests that cover those
    guards route the bad value through here, so the intent is explicit and the
    escape is greppable rather than scattered as `# type: ignore`.
    """
    return value
...
def get_layer(adata: AnnData, key: str) -> NDArray[np.number]:
    ...
    `layers` values carry the same wide union as `.X`, so they need narrowing
    before numpy will accept them.
    """
...
def get_layer_csr(adata: AnnData, key: str) -> csr_matrix:
    ...
    For the tests that compare ``.data``, i.e. the stored values rather than the
    dense contents.
    """
